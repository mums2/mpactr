...
Using the `get_cv_data()` on a mpactr object when "use_recursive" is set to true, will result in the "used_recursion" column added to the same dataframe.

```{r}
data_rep_recursive <- filter_cv(data,
  cv_threshold = 0.2,
  copy_object = TRUE,
  is_recursive = TRUE
)

cv_data_recursive <- get_cv_data(data_rep_recursive)
head(cv_data_recursive)
```

If a feature passed the cv filter but was flagged for using "use_recursive", we know that it only passed because of the recursive method.

```{r}
head(cv_data_recursive[which(cv_data_recursive$PassesCvFilter == TRUE & cv_data_recursive$used_recursion == TRUE),])
```

Running the same plot as above but with "use_recursive" being true will look like this:
