...
test_that("parse_ions_by_group flags the correct ions", {
 mpactr_class <- mpactr$new(test_path("exttestdata","102623_peaktable_coculture_simple.csv"),
  ...
  ang_18 <-  read_csv(test_path("exttestdata", "output_ANG18_monoculture.csv"), col_names = c("V1"),
                      show_col_types = FALSE)
  angdt <- read_csv(test_path("exttestdata", "output_ANGDT_monoculture"), col_names = c("V1"),
                    show_col_types = FALSE)
