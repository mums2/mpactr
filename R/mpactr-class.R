mpactr <- R6Class("mpactr", public = list(
  ...
  initialize = function(peak_table_path, meta_data_path) {
    self$peak_table = data.table(readr::read_csv(peak_table_path, skip = 2, show_col_types = FALSE))
    self$meta_data = data.table(readr::read_csv(meta_data_path, show_col_types = FALSE))
