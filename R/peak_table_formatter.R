...
metaboscape_formatter <- function(peak_table, sample_names){
    ...
    peak_table_convert <- with(peak_table_convert, peak_table_convert[ , ion := gsub(".*\\[(.+)\\].*", "\\1",
                                                                                       ADDUCT)][
    ...
    peak_table_mpactr <- with(peak_table_convert, peak_table_convert[ , .SD, .SDcols = c("Compound", "mz", "rt",
                                                                                        sample_names)])

    return(list("peak_table" = peak_table_mpactr,
