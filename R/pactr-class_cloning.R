...
clone <- function(mpactr_object)
{
  ...
  new_object$mpactr_data$set_peak_table(data.table::copy(new_object$mpactr_data$get_peak_table()))
  new_object$logger <- as.environment(as.list(mpactr_object$logger, all.names=TRUE))
  return(new_object)
}
